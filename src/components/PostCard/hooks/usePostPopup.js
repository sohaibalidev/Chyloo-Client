...
export const usePostPopup = (post) => {
  ...
  useEffect(() => {
    ...
    observerRef.current = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (!videoId || !videoId.startsWith(`post-popup-${post._id}`)) return;
    ...
    Object.entries(videoRefs.current).forEach(([videoId, video]) => {
      if (video && videoId.startsWith(`post-popup-${post._id}`)) {
  ...
  useEffect(() => {
    if (post?._id) {
      mediaContainerRef.current = document.getElementById(`post-popup-media-container-${post._id}`);
    }
  }, [post]);
  ...
  const toggleVideoPlayback = (mediaIndex) => {
    const videoId = `post-popup-${post._id}-${mediaIndex}`;
    const video = videoRefs.current[videoId];
